...
def _day_to_dict(day) -> Dict[str, Any]:
    """Convert a DayComparison into the dict format expected by create_multi_day_events.

    Args:
        day: DayComparison-like object exposing the 9 charging attributes

    Returns:
        Dict with the 9 keys consumed by create_multi_day_events
    """
    return {
        "date": day.date,
...
def sync_plan_to_calendar(plan, cal_config: Dict[str, Any]) -> bool:
    """Sync an already-built multi-day plan to Google Calendar.

    Args:
        plan: MultiDayPlan-like object exposing .days, .kwh_amount, .best_day
        cal_config: The "google_calendar" config section
    ...
    try:
        ...
        days_data = [_day_to_dict(day) for day in plan.days]
